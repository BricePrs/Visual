...
public:
    ...
    Camera(double aspect) : mPitch(0.), mYaw(0.), mYFov(glm::radians(95.)), mPosition(glm::vec3(0.)), mNear(0.01), mFar(100.),
        mAspect(aspect), mFront(glm::vec3(0., 0., 1.)), mUp(glm::vec3(0., 1., 0.)), mRight(glm::vec3(1., 0., 0.)) {}

    Camera(uint32_t width, uint32_t height) : mPitch(0.), mYaw(0.), mYFov(glm::radians(95.)), mPosition(glm::vec3(0.)), mNear(0.01), mFar(100.),
